...
export const tableType: Record<string, (args: { parentRow: MRT_Row<EntityBase & AnyObject>; collection: keyof typeof collections; objectType: string; propertyName: string }) => TableTypeObject<any>> =
    {
        ...
        list: <T extends AnyObject | string | number | boolean | Date>({
            ...
            getRowId: ((row: { index: number }) => row.index.toFixed(0)) as any,
            invalidator: {
                onSuccess: async () => {
                    console.error('INVALIDATING', [collection]);
        ...
        collection: <T extends EntityBase>({ collection }: { collection: keyof typeof collections }) => ({
            ...
            queryFn: () =>
                Promise.resolve(
                    Array.from(window.$$store?.objects<T>(collection) ?? []).sort((a, b) => {
                        return a._id < b._id ? 1 : a._id > b._id ? -1 : 0;
                    })
                ),
            columns: collections[collection].getColumns(),
            getRowId: (row: EntityBase) => fromOID(row._id),
            insert: async (args: { values: AnyObject }) => {
                const sku = await ($initialCollection['sku']() as Promise<ISku>);
                const prod = await ($initialCollection['product']() as Promise<IProduct>)
                const convertSku = $convertToRealm['sku'];
                const convertProduct = $convertToRealm['product'];
                return new Promise<T>((resolve, reject) => {
                    try {
                        const func = () => {
                            if (window.$$store == null) throw new Error('NO REALM');
                            if (collection === 'product') {
                                const productObject = { ...prod, ...(args.values as any), folder: prod.folder.toHexString(false) };
                                console.log(`productObject`, productObject);
                                console.log(`convertProduct(productObject)`, convertProduct(productObject));
                                const productResult = window.$$store.create<IProduct>('product', convertProduct(productObject));   
                                productResult.update();                             
                                console.log(`productResult`, productResult);
                                const skuResult = window.$$store.create<ISku>('sku', convertSku({ ...sku, product: productResult as any } as any), Realm.UpdateMode.Modified);
                                skuResult.update();
                                console.log(`skuResult`, skuResult);
                                resolve(skuResult as any)
                            } else {
                                const result = window.$$store?.create(collection, args.values as any, Realm.UpdateMode.Modified);
