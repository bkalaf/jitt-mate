...
export function RunUpdateButton<T extends EntityBase>({ table }: { table: Table<T>; }) {
    ...
    const onClick = useCallback(() => {
        ...
        const func = () => {
            Object.entries(table.getState().rowSelection).filter(x => x[1]).map((x) => toOID(x[0])).filter(x => x != null).map(x => db.objectForPrimaryKey<IRealmEntity>(collection, x as any)).forEach(obj => obj?.update(db));
        };
        checkTransaction(db)(func);
        invalidate.onSuccess();
    }, [db, invalidate, table]);
    const canClick = useCallback(() => table.getIsSomeRowsSelected(), [table]);
    return <Button icon={faExclamationCircle} onClick={onClick} renderCondition={true} disabledCondition={not(canClick)} className='w-6 h-6 bg-transparent border border-sky-500 text-rose-700' />;
}
