...
export function ReactTable<T>({
    ...
    table: Table<T>;
    SubComponent: SubComponentFunction<T>;
    getId: (x: T) => string;
    setChildren: StateSetter<JSX.Element | null>;
}) {
                ...
                <table className='relative w-full h-full p-0 overflow-auto table-auto border-seperate'>
                    ...
                    <tbody className='w-auto h-full'>
                        {table.getRowModel().rows.map((row) => (
                            <TableRow key={row.id} SubComponent={SubComponent as any} row={row as any} table={table as any} getId={getId as any} />
                        ))}
                    </tbody>
