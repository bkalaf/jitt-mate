...
export function InsertForm<T, TValue>({ onSuccess, getId, collectionName, table, resultant }: { collectionName: string; table: Table<T>; resultant?: (x: TValue) => void; onSuccess?: (x: TValue) => void; getId: (x: T) => string }) {
    const insertRecord = useInsertRecord(collectionName);
    const logger = useLogger()
    return (
        <FormProvider submit={compR(handlePayload)(insertRecord)} resultant={resultant} onSuccess={onSuccess}>
            <div className='grid grid-cols-2 gap-x-3 gap-y-1'>
                ...
                {table
                    ...
                    .filter((x) => {
                        ...
                        return !['editRow', 'expandRow'].includes((x.columnDef as any).accessorKey ?? x.columnDef.id ?? '');
                    })
                    .map((column, ix) => {
