import { calculateCheckDigit } from '../barcode';
import { importApparelType, importBacklineType, importCollarType, importCuffType, importGender, importLegType, importMaterials, importNecklineTypes, importOrigin, importSize, importSleeveType, importTopAdornments, importWaistType, LegTypeKeys, SizeKeys } from '../enums/importNecklineType';
...
import { IProduct, ISku, suffix } from './db';
import { toKVP } from '../posting/toKVP';
...
export const getSizeName = tap<[SizeKeys | undefined], ISizeEntry | undefined, string | undefined>((x?: any) => undefined, (x?: ISizeEntry) => x?.name);
export const getSizeSelector = tap<[SizeKeys | undefined], ISizeEntry | undefined, string | undefined>((x?: any) => undefined, (x?: ISizeEntry) => x?.selector);
export const getSizeKey = tap<[SizeKeys | undefined], ISizeEntry | undefined, string | undefined>((x?: any) => undefined, (x?: ISizeEntry) => x?.key);

export const toFlag = (name: string, desiredResult = true) => (value?: boolean) => (value ?? false) === desiredResult ? ['* ', name, '*'].join('') : undefined;

export function getDescription(product: IProduct, sku: ISku) {
    const { brand, chestIn, classifier, color, cutNo, descriptiveText, heightIn, inseamIn, lengthIn, madeOf, pocketCount, modelNo, neckIn, origin, rn, size, sleeveIn, sleeveType, styleNo, torsoIn, upcs, widthIn, waistIn, weightG, features, necklineType, backlineType, waistType, topAdornment, hasTags, isRare, isVintage, cuffType, collarType, circa } = product;
    const { defects } = sku;
    ...
    const $upcs = upcs.length > 0 ? `UPC: ${upcs.map(u => {
    ...
    const $shortDescription = toProperCase([$gender, color, $sleeveType, $pocket, $necklineType, $backlineType, $waistType, $legType, $collarType, $cuffType, $topAdornment, descriptiveText, $apparelType].filter(x => x != null).join(' ')).replaceAll('  ', '').trim();
    ...
    const dimValues = ([lengthIn, widthIn, heightIn].filter(x => x != null) as number[]).map((n: number) => Number.isInteger(n) ? n.toFixed(0) : n.toFixed(2)).map(s => [s, '"'].join('')).join('x');
    const $dims = hasDims ? [`DIMS (${dimKeys})`, dimValues].join(': ') : undefined;
    const $madeOf = madeOf != null ? Object.entries(madeOf).length > 0 ? ['MADE OF: \n', Object.entries(madeOf).map(([k, v]) => ['\t', [importMaterials(k)[1], [(v * 100).toFixed(0), '%'].join('')].join(': ')].join('')).join('\n')].join('') : undefined : undefined;
    ...
    const $narrative = ([$title, $upcs, $size, $model, $style, $cut, $circa, $neckline, $backline, $waistT, $collar, $cuff, $neck, $sleeve, $chest, $waist, $inseam, $torso, $weight, $rn, $origin, $dims, $madeOf, $features, $defects, $flags].filter(x => x != null && x.length > 0) as string[]).join('\n');
